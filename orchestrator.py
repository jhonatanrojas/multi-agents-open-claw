...
async def relay_team_messages(client: OpenClawClient) -> None:
    ...
    coordination_messages = _compact_coordination_messages(arch_messages)
    project_identity = str(
        (mem.get("project", {}) or {}).get("id")
        or (mem.get("project", {}) or {}).get("name")
        or "project"
    )
    coordination_digest = hashlib.sha1(
        json.dumps(coordination_messages, ensure_ascii=False, sort_keys=True).encode("utf-8")
    ).hexdigest()[:12]
